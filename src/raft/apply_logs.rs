...
use crate::{
    common::{ApplyLogsTask, DependencyAddr, CLIENT_RPC_TX_ERR},
    messages::{ClientError, ClientPayloadResponse, Entry},
    network::RaftNetwork,
    raft::Raft,
    storage::{ApplyEntryToStateMachine, GetLogEntries, RaftStorage, ReplicateToStateMachine},
    ...
    Raft<D, R, E, N, S>
{
    ...
    pub(super) fn process_apply_logs_task(
        &mut self,
        ctx: &mut Context<Self>,
        msg: ApplyLogsTask<D, R, E>,
    ) -> impl ActorFuture<Actor = Self, Item = (), Error = ()> {
        match msg {
            ApplyLogsTask::Entry { entry, chan } => {
                fut::Either::A(self.process_apply_logs_task_with_entries(ctx, entry, chan))
            }
            ApplyLogsTask::Outstanding => {
                fut::Either::B(self.process_apply_logs_task_outstanding(ctx))
            }
        }
    }
    ...
    fn process_apply_logs_task_with_entries(
        ...
        let f = if (self.last_applied + 1) != entry_index {
            fut::Either::A(
                fut::wrap_future(self.storage.send::<GetLogEntries<D, E>>(GetLogEntries::new(
                    self.last_applied + 1,
                    entry_index,
                )))
                ...
                .and_then(|res, act, ctx| {
                    act.map_fatal_storage_result(ctx, res)
                        .map_err(|_, _, _| ClientError::Internal)
                })
                .and_then(|res, act, _| {
                    let line_index = res.iter().last().map(|e| e.index);
                    fut::wrap_future(
                        act.storage.send::<ReplicateToStateMachine<D, E>>(
                            ReplicateToStateMachine::new(res),
                        ),
                    )
                    .map_err(|err, act: &mut Self, ctx| {
                        act.map_fatal_actix_messaging_error(ctx, err, DependencyAddr::RaftStorage);
        ...
        f.and_then(move |_, act, _| {
            fut::wrap_future(
                act.storage.send::<ApplyEntryToStateMachine<D, R, E>>(
                    ApplyEntryToStateMachine::new(entry),
                ),
            )
            .map_err(|err, act: &mut Self, ctx| {
                act.map_fatal_actix_messaging_error(ctx, err, DependencyAddr::RaftStorage);
        ...
        .then(move |res, act, _| match res {
            Ok(data) => {
                ...
                if let Some(tx) = chan {
                    let _ = tx
                        .send(Ok(ClientPayloadResponse::Applied {
                            index: line_index,
                            data,
                        }))
    ...
    /// then apply them.
    fn process_apply_logs_task_outstanding(
        &mut self,
        _: &mut Context<Self>,
    ) -> impl ActorFuture<Actor = Self, Item = (), Error = ()> {
        // Guard against no-op.
        ...
        fut::Either::B(
            fut::wrap_future(
                self.storage
                    .send::<GetLogEntries<D, E>>(GetLogEntries::new(start, stop)),
            )
            .map_err(|err, act: &mut Self, ctx| {
                act.map_fatal_actix_messaging_error(ctx, err, DependencyAddr::RaftStorage)
            })
            ...
            .and_then(|entries, act, _| {
                ...
                fut::wrap_future(
                    act.storage.send::<ReplicateToStateMachine<D, E>>(
                        ReplicateToStateMachine::new(entries),
                    ),
                )
                .map_err(|err, act: &mut Self, ctx| {
                    act.map_fatal_actix_messaging_error(ctx, err, DependencyAddr::RaftStorage)
