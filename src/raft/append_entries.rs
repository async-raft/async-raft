...
impl<D: AppData, R: AppDataResponse, E: AppError, N: RaftNetwork<D>, S: RaftStorage<D, R, E>>
    Handler<AppendEntriesRequest<D>> for Raft<D, R, E, N, S>
{
    ...
    fn handle(&mut self, msg: AppendEntriesRequest<D>, ctx: &mut Self::Context) -> Self::Result {
        ...
        if &msg.term < &self.current_term {
            return Box::new(fut::ok(AppendEntriesResponse {
        ...
        if &self.commit_index > &self.last_applied {
            let _ = self
                .apply_logs_pipeline
                .unbounded_send(ApplyLogsTask::Outstanding);
        }
        ...
        let (term, msg_prev_index, msg_prev_term) =
            (self.current_term, msg.prev_log_index, msg.prev_log_term);
        let has_prev_log_match = &msg.prev_log_index == &u64::min_value()
            || (&msg_prev_index == &self.last_log_index && &msg_prev_term == &self.last_log_term);
        if has_prev_log_match {
            return Box::new(self.append_log_entries(ctx, Arc::new(msg.entries)).map(
                move |_, _, _| AppendEntriesResponse {
                    term,
                    success: true,
                    conflict_opt: None,
                },
        ...
        Box::new(
            self.log_consistency_check(ctx, msg_prev_index, msg_prev_term)
                .and_then(move |res, act, ctx| match res {
                    Some(conflict_opt) => fut::Either::A(fut::ok(AppendEntriesResponse {
                        term,
                        success: false,
                        conflict_opt: Some(conflict_opt),
                    })),
                    None => fut::Either::B(act.append_log_entries(ctx, Arc::new(msg.entries)).map(
                        move |_, _, _| AppendEntriesResponse {
                            term,
                            success: true,
                            conflict_opt: None,
    ...
    Raft<D, R, E, N, S>
{
    ...
    fn append_log_entries(
        ...
        fut::Either::B(f.and_then(move |_, act, _| {
            ...
            fut::wrap_future(
                act.storage
                    .send::<ReplicateToLog<D, E>>(ReplicateToLog::new(entries.clone())),
            )
    ...
    fn log_consistency_check(
        ...
        fut::wrap_future(
            self.storage
                .send::<GetLogEntries<D, E>>(GetLogEntries::new(index, index)),
        )
        .map_err(|err, act: &mut Self, ctx| {
            act.map_fatal_actix_messaging_error(ctx, err, DependencyAddr::RaftStorage)
        })
        ...
        .and_then(move |res, act, _| {
            match res.last() {
                ...
                Some(target_entry) => {
                    let (target_entry_index, target_entry_term) =
                        (target_entry.index, target_entry.term);
                    if &target_entry_index == &index && &target_entry_term == &term {
                        ...
                        fut::Either::B(
                            fut::wrap_future(
                                storage
                                    .send::<GetLogEntries<D, E>>(GetLogEntries::new(start, index)),
                            )
                            .map_err(|err, act: &mut Self, ctx| {
