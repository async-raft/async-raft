...
use crate::{
    ...
    network::RaftNetwork,
    replication::{RSState, ReplicationStream},
    storage::RaftStorage,
    ...
    ReplicationStream<D, R, E, N, S>
{
    ...
    pub(super) fn drive_state_line_rate(&mut self, ctx: &mut Context<Self>) {
        ...
        if state.buffered_outbound.len() > 0 {
            ...
            // Send the payload.
            let f = self
                .send_append_entries(ctx, payload)
                // Process the response.
                .and_then(move |res, act, ctx| {
                    act.handle_append_entries_response(ctx, res, last_index_and_term)
                })
                ...
                .then(|res, act, ctx| {
                    ...
                    match res {
                        ...
                        Err(_) => {
                            fut::Either::B(act.transition_to_lagging(ctx).then(|res, act, ctx| {
                                act.drive_state(ctx);
                                fut::result(res)
                            }))
                        }
