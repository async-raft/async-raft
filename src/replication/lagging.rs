...
use crate::{
    ...
    network::RaftNetwork,
    replication::{RSRateUpdate, RSState, ReplicationStream},
    ...
    ReplicationStream<D, R, E, N, S>
{
    ...
    pub(super) fn drive_state_lagging(&mut self, ctx: &mut Context<Self>) {
        ...
        let batch_will_reach_line = (self.next_index > self.line_index)
            || ((self.line_index - self.next_index) < self.config.max_payload_entries);
        ...
        if let SnapshotPolicy::LogsSinceLast(threshold) = &self.config.snapshot_policy {
            if self.line_index > self.match_index
                && (self.line_index - self.match_index) >= *threshold
            {
                debug!(
                    "{} sees {} as too far behind. Needs snapshot.",
                    self.id, self.target
                );
                let f = self
                    .transition_to_snapshotting(ctx)
                    .and_then(|_, act, ctx| {
        ...
        ctx.spawn(
            ...
            .and_then(move |stop, act: &mut Self, _| {
                fut::wrap_future(act.storage.send(GetLogEntries::new(start, stop))).map_err(
                    |err, act: &mut Self, ctx| {
            ...
            .and_then(move |entries, act, ctx| {
                ...
                let payload = AppendEntriesRequest {
                    ...
                    leader_id: act.id,
                    prev_log_index,
                    prev_log_term, // NOTE: these are moved in from above.
                    entries,
                    leader_commit: act.line_commit,
                };
                fut::Either::B(act.send_append_entries(ctx, payload).and_then(
                    move |res, act, ctx| {
                        act.handle_append_entries_response(ctx, res, last_log_and_index)
                    },
                ))
            })
            // Transition to line rate if needed.
            .and_then(|_, act, ctx| match &act.state {
            ...
            .then(|res, _, _| match res {
                ...
                Err(err) => {
                    ...
                    fut::Either::B(fut::wrap_future(Delay::new(delay).map_err(|_| ()).then(
                        move |res| match res {
                            Ok(_) => Err(err),
                            Err(_) => Err(err),
                        },
