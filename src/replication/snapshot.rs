...
use actix::prelude::*;
use futures::{sink::Sink, sync::mpsc};
use log::error;
...
use crate::{
    ...
    network::RaftNetwork,
    replication::{
        RSNeedsSnapshot, RSNeedsSnapshotResponse, RSRevertToFollower, RSState, RSUpdateMatchIndex,
        ReplicationStream,
    },
    storage::RaftStorage,
    ...
    ReplicationStream<D, R, E, N, S>
{
    ...
    pub(super) fn drive_state_snapshotting(&mut self, ctx: &mut Context<Self>) {
        ...
        ctx.spawn(
            fut::wrap_future(self.raftnode.send(RSNeedsSnapshot))
        ...
        fut::wrap_stream(snap_stream)
            ...
            .then(move |res, act, _| match res {
                ...
                Ok(_) => {
                    ...
                    act.raftnode.do_send(RSUpdateMatchIndex {
                        target: act.target,
                ...
                Err(_) => {
                    error!(
                        "Snapshot stream finished with an error. Delaying before next iteration."
                    );
...
impl SnapshotStream {
    ...
    fn run(mut self) {
        ...
        let file_and_len =
            File::open(&self.file).and_then(|file| file.metadata().map(|meta| (file, meta.len())));
        ...
        loop {
            ...
            let chunksize = if self.bufsize > remaining {
                remaining
            } else {
                self.bufsize
            };
