...
use fixtures::{
    ...
    setup_logger, ClientRequest, Node, RaftTestController,
};
        ...
        Box::new(|act, ctx| {
            ...
            ctx.spawn(
                fut::wrap_future(act.network.send(GetCurrentLeader))
                    .map_err(|_, _: &mut RaftTestController, _| {
                        panic!("Failed to get current leader.")
                    })
                    .and_then(|res, _, _| fut::result(res))
                    .and_then(|leader_opt, _, _| {
                        let leader =
                            leader_opt.expect("Expected the cluster to have elected a leader.");
                        fut::ok(leader)
                    })
                    // Send 100 requests as fast as possible to the current leader & give the cluster 1 second to do work.
                    .and_then(|leader, _, ctx| {
                        for idx in 0..100 {
                            ctx.notify(ClientRequest {
                                payload: idx,
                                current_leader: Some(leader),
                                cb: None,
                            });
                        }
                        fut::wrap_future(Delay::new(Instant::now() + Duration::from_secs(1)))
                            .map_err(|_, _, _| ())
                    ...
                    .and_then(|leader, act, _| {
                        act.network
                            .do_send(ExecuteInRaftRouter(Box::new(move |act, _| {
                                act.isolate_node(leader)
                            })));
                    ...
                    .and_then(|(current_leader, old_leader), act, ctx| {
                        ...
                        fut::wrap_future(Delay::new(Instant::now() + Duration::from_secs(10)))
                            .map_err(|_, _, _| ())
                    })
                    ...
                    .and_then(|_, act, _| {
                        act.network.do_send(ExecuteInRaftRouter(Box::new(|act, _| {
                            ...
                            let leader = data
                                ...
                                .expect("Expected leader to exist.");
                            assert!(
                                data.iter().all(|e| e.current_leader == Some(leader.id)),
                                "Expected all nodes have the same leader."
                            );
                            assert!(
                                data.iter().all(|e| e.current_term == leader.current_term),
                                "Expected all nodes to be at the same term."
                            );
                            assert!(
                                data.iter()
                                    .all(|e| e.last_log_index == leader.last_log_index),
                                "Expected all nodes have last log index."
                            );
                            assert!(
                                data.iter().all(|e| e.last_applied == leader.last_applied),
                                "Expected all nodes have the same last applied value."
                            );
                        })));
                        fut::ok(())
                    })
                    ...
                    .and_then(move |_, _, _| {
                        ...
                        fut::wrap_future(storage0.send(GetCurrentState))
                            .map_err(|err, _: &mut RaftTestController, _| panic!(err))
                            .and_then(|res, _, _| fut::result(res))
                            .and_then(move |s0, _, _| {
                                fut::wrap_future(storage1.send(GetCurrentState))
                                    .map_err(|err, _, _| panic!(err))
                                    .and_then(|res, _, _| fut::result(res))
                                    .and_then(move |s1, _, _| {
                                        fut::wrap_future(storage2.send(GetCurrentState))
                                            .map_err(|err, _, _| panic!(err))
                                            .and_then(|res, _, _| fut::result(res))
                                            .and_then(move |s2, _, _| fut::ok((s0, s1, s2)))
                                    })
                            })
                            ...
                            .and_then(|(s0, s1, s2), _, _| {
                                // Hard state.
                                assert_eq!(
                                    s0.hs.current_term, s1.hs.current_term,
                                    "Expected hs current term for nodes 0 and 1 to be equal."
                                );
                                assert_eq!(
                                    s0.hs.current_term, s2.hs.current_term,
                                    "Expected hs current term for nodes 0 and 2 to be equal."
                                );
                                assert_eq!(
                                    s0.hs.membership, s1.hs.membership,
                                    "Expected hs membership for nodes 0 and 1 to be equal."
                                );
                                assert_eq!(
                                    s0.hs.membership, s2.hs.membership,
                                    "Expected hs membership for nodes 0 and 2 to be equal."
                                );
                                // Log.
                                assert_eq!(
                                    s0.log, s1.log,
                                    "Expected log for nodes 0 and 1 to be equal."
                                );
                                assert_eq!(
                                    s0.log, s2.log,
                                    "Expected log for nodes 0 and 2 to be equal."
                                );
                                // Snapshot data.
                                assert_eq!(
                                    s0.snapshot_data, s1.snapshot_data,
                                    "Expected snapshot_data for nodes 0 and 1 to be equal."
                                );
                                assert_eq!(
                                    s0.snapshot_data, s2.snapshot_data,
                                    "Expected snapshot_data for nodes 0 and 2 to be equal."
                                );
                                // State machinen data.
                                assert_eq!(
                                    s0.state_machine, s1.state_machine,
                                    "Expected state machines for nodes 0 and 1 to be equal."
                                );
                                assert_eq!(
                                    s0.state_machine, s2.state_machine,
                                    "Expected state machines for nodes 0 and 2 to be equal."
                                );
                                fut::ok(())
                            })
                            .and_then(|_, _, ctx| {
                                ctx.run_later(Duration::from_secs(2), |_, _| {
                                    System::current().stop()
                                });
                                fut::ok(())
                            })
