        ...
        Box::new(|act, ctx| {
            ...
            let task = act
                ...
                .and_then(|old_leader, _, _| {
                    fut::wrap_future(Delay::new(Instant::now() + Duration::from_secs(5)))
                        .map_err(|_, _, _| ())
                        .map(move |_, _, _| old_leader)
                })
                // Write enough data to trigger a snapshot. Won't proceed until data is finished writing.
                .and_then(|old_leader, act, ctx| {
                    act.write_above_snapshot_threshold(ctx)
                        .map(move |_, _, _| old_leader)
                })
                // Restore old node.
                ...
                .and_then(|_, _, _| {
                    fut::wrap_future(Delay::new(Instant::now() + Duration::from_secs(15)))
                        .map_err(|_, _, _| ())
                })
...
impl RaftTestController {
    fn isolate_leader(
        &mut self,
        _: &mut Context<Self>,
    ) -> impl ActorFuture<Actor = Self, Item = NodeId, Error = ()> {
    ...
    fn write_above_snapshot_threshold(
        &mut self,
        _: &mut Context<Self>,
