...
fn initialization() {
    ...
    ctl.start_with_test(
        ...
        Box::new(|act, ctx| {
            ...
            let task = fut::wrap_future(act.network.send(ExecuteInRaftRouter(Box::new(
                move |act, _| {
                    for node in act.metrics.values() {
                        assert!(
                            node.state == State::NonVoter,
                            "Expected all nodes to be in NonVoter state at beginning of test."
                        );
                        assert_eq!(
                            node.last_log_index, 0,
                            "Expected all nodes to be at index 0 at beginning of test."
                        );
                    }
                },
            ))))
            ...
            .and_then(|_, act, _| {
                ...
                cmds.push(Box::new(
                    act.nodes
                        .get(&0)
                        .expect("Expected node 0 to be registered with test controller.")
                        .send(InitWithConfig::new(vec![0, 1, 2]))
                ...
                cmds.push(Box::new(
                    act.nodes
                        .get(&1)
                        .expect("Expected node 1 to be registered with test controller.")
                        .send(InitWithConfig::new(vec![0, 1, 2]))
                        .map_err(|err| panic!(err))
                        .and_then(|res| res),
                ));
                cmds.push(Box::new(
                    act.nodes
                        .get(&2)
                        .expect("Expected node 2 to be registered with test controller.")
                        .send(InitWithConfig::new(vec![0, 1, 2]))
                        .map_err(|err| panic!(err))
                        .and_then(|res| res),
                ));
            ...
            .and_then(|_, _, _| {
                fut::wrap_future(
                    Delay::new(Instant::now() + Duration::from_secs(5)).map_err(|_| ()),
                )
            })
            ...
            .and_then(|_, _, _| {
                fut::wrap_future(
                    Delay::new(Instant::now() + Duration::from_secs(2)).map_err(|_| ()),
                )
            })
            .and_then(|_, act, _| {
                fut::wrap_future(
                    act.network
                        .send(ExecuteInRaftRouter(Box::new(move |act, _| {
                            ...
                            assert!(
                                act.metrics
                                    .values()
                                    .all(|node| node.current_leader == Some(leader.id)),
                                "Expected all nodes to agree upon current leader."
                            );
                            assert!(
                                act.metrics
                                    .values()
                                    .all(|node| node.current_term == leader.current_term),
                                "Expected all nodes to agree upon current term."
                            );
                            assert!(
                                act.metrics
                                    .values()
                                    .all(|node| node.last_log_index == leader.last_log_index),
                                "Expected all nodes to be at the same index."
                            );
                            assert!(
                                act.metrics
                                    .values()
                                    .all(|node| node.membership_config == leader.membership_config),
                                "Expected all nodes to have same cluster config."
                            );
